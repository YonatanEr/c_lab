...
void initSocket(int *port_ptr, int *this_sock_ptr) {
  ...
  do {
    ...
    *port_ptr =
        (1 + LOWER_PORT_BOUND) +
        ((te.tv_sec * te.tv_usec) % (UPPER_PORT_BOUND - LOWER_PORT_BOUND - 1));
...
int acceptConnection(int lb_socket) {
  ...
  int client_socket =
      accept(lb_socket, (struct sockaddr *)&address, &address_size);
  assert(client_socket >= 0);
  return client_socket;
}

void waitForConnections(LoadBalancer *lb) {
